...
def test_print_signposts_true(test_csv_content, mocker):
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".csv", delete=False
    ) as input_file:
        input_file.write(test_csv_content)
        ...
        with tempfile.NamedTemporaryFile(
            ...
            try:
                ...
                with open(output_file.name, "r") as f:
                    ...
                    # Ensure that signpost rows ARE present when print_signposts=True
                    assert len(signposts) > 0, "Signpost rows should be present when print_signposts=True"
...
def test_print_signposts_false_with_ignore_signposts(test_csv_content, mocker):
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".csv", delete=False
    ) as input_file:
        input_file.write(test_csv_content)
        ...
        with tempfile.NamedTemporaryFile(
            ...
            try:
                ...
                with open(output_file.name, "r") as f:
                    ...
                    # Ensure that signpost rows are NOT present when print_signposts=False, even with ignore_signposts=True
                    assert len(signposts) == 0, "Signpost rows should be filtered out when print_signposts=False"
