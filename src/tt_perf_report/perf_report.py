...
def filter_by_signpost(df, signpost=None, ignore_signposts=False):
    ...
    if ignore_signposts:
        print(
            colored(
                "Ignoring all signposts. Using the entire file for analysis.", "cyan"
            )
        )
        return df
    ...
    if signpost:
        ...
        print(
            colored(
                f"Specified signpost '{signpost}' not found. Defaulting to the last signpost.",
                "yellow",
            )
        )
...
def evaluate_fidelity(
    ...
    if in0_bits == 8 and out_bits >= 7:
        ...
        if math_fidelity == "LoFi":
            ...
            return (
                "too_high",
                "LoFi is sufficient with BFP4 weights, use it for much higher throughput",
            )
    else:
        print(
...
def analyze_matmul(row, csv_format="v2"):
    ...
    is_dram_sharded = (
        "MatmulMultiCoreReuseMultiCastDRAMShardedProgramConfig" in attributes
    )
    ...
    W, Z = get_value_physical_logical(
        row[get_column_name("INPUT_0_W", csv_format)]
    ), get_value_physical_logical(row[get_column_name("INPUT_0_Z", csv_format)])
    ...
    dram_percentage = (
        (dram_speed_gb_s / 288) * 100 if dram_speed_gb_s is not None else None
    )
...
def analyze_halo(row):
    ...
    try:
        memory_layout = (
            attributes.split("memory_layout=")[1].split(";")[0].split("::")[1]
        )
    except (IndexError, AttributeError):
        memory_layout = "x"
...
def analyze_conv(row, csv_format="v2"):
    ...
    W = [
        int(x)
        for x in (attributes.split("window_hw")[1].split("; ")[0][2:-1].split(";"))
    ]
    ...
    try:
        act_block_h_ntiles = int(
            attributes.split("act_block_h_ntiles")[1][1:].split(";")[0]
        )
    ...
    try:
        enable_act_double_buffer = (
            "true" == attributes.split("enable_act_double_buffer': '")[1].split("'")[0]
        )
    except (IndexError, ValueError):
    ...
    try:
        enable_split_reader = (
            "true" == attributes.split("enable_split_reader': '")[1].split("'")[0]
        )
    except (IndexError, ValueError):
        enable_split_reader = "x"

    try:
        per_core_out_matrix_height_ntile = int(
            attributes.split("per_core_out_matrix_height_ntile")[1][1:].split(";")[0]
        )
    except (IndexError, ValueError):
...
def analyze_op(row, prev_row, csv_format="v2"):
    ...
    device_time = Cell(
        (
            row["DEVICE KERNEL DURATION [ns]"] / 1000
            if pd.notna(row["DEVICE KERNEL DURATION [ns]"])
            else None
        ),
        unit="us",
        decimals=0,
    ...
    if prev_row is not None and pd.notna(prev_row["OP TO OP LATENCY [ns]"]):
        op_to_op_gap = Cell(
            (
                ...
                if pd.notna(row["OP TO OP LATENCY [ns]"])
                else None
            ),
            unit="us",
            decimals=0,
    ...
    math_fidelity = ""
    math_fidelity += (
        f"{short_name(input_0_datatype)}" if pd.notna(input_0_datatype) else ""
    )
    math_fidelity += (
        f", {short_name(input_1_datatype)}" if pd.notna(input_1_datatype) else ""
    )
    math_fidelity += (
        f" => {short_name(output_datatype)}" if pd.notna(output_datatype) else ""
    )
    ...
    if "Matmul" in op_code.raw_value:
        ...
        flops = Cell(
            flops / 1e12 if pd.notna(flops) else None, unit="TFLOPs", decimals=1
        )
    ...
    input_0_memory = Cell(
        row["INPUT_0_MEMORY"] if pd.notna(row["INPUT_0_MEMORY"]) else None
    )
...
def add_derived_columns(rows):
    total_duration = sum(
        ...
        op_data["Op-to-Op Gap"].raw_value
        for op_data in rows
        if op_data["Op-to-Op Gap"].raw_value is not None
    )
    for op_data in rows:
        device_time = (
            op_data["Device Time"].raw_value
        ...
        op_data["Total %"] = Cell(
            ((device_time + op_to_op_gap) / total_duration) * 100, unit="%", decimals=1
        )
        if (
            op_data["Device Time"].raw_value is None
            and op_data["Op-to-Op Gap"].raw_value is None
        ):
...
def print_row(row, col_widths, headers):
    def format_cell(header, cell):
        ...
        return pad_string(
            text,
            col_widths[headers.index(header)],
...
def color_row(op_data, percentage, min_percentage):
    if percentage is not None and percentage < min_percentage:
        ...
        if (
            op_data["Op-to-Op Gap"].raw_value is not None
            and op_data["Op-to-Op Gap"].raw_value > 6.5
        ):
            op_data["Op-to-Op Gap"].color = "red"

        if (
            "Matmul" in op_data["OP Code"].raw_value
            or "OptimizedConvNew" in op_data["OP Code"].raw_value
        ) and op_data["Math Fidelity"].raw_value:
...
def print_performance_table(rows, headers, col_widths, device_ops, host_ops):
    ...
    print(
        "  ".join(
            pad_string(header, col_widths[i], align="left")
    ...
    print_row(
        {
            k: Cell(v.raw_value, v.unit, v.decimals, color="grey")
...
def print_fallback_advice(rows, headers, col_widths):
    host_ops = [
        op_data for op_data in rows if "(torch)" in op_data["OP Code"].raw_value
    ]
    if host_ops:
        print("Fallback\n--------")
...
def print_op_to_op_gap_advice(rows, headers, col_widths):
    ...
    if high_gap_ops:
        ...
        for idx, op_data in high_gap_ops:
            print_row(op_data, col_widths, headers)
        max_gap_overhead = sum(
            op_data["Op-to-Op Gap"].raw_value - 6 for _, op_data in high_gap_ops
        )
...
def generate_matmul_advice(op_data):
    ...
    math_fidelity = (
        op_data["Math Fidelity"].raw_value.split()[0]
        if op_data["Math Fidelity"].raw_value
        else None
    )
    output_datatype = op_data["Output Datatype"].raw_value
    input_0_datatype = op_data["Input 0 Datatype"].raw_value
    ...
    if op_data["Bound"].raw_value in ["DRAM", "BOTH"]:
        ...
        if input_0_memory and "L1" not in input_0_memory:
            advice.append(
                f"If possible place input 0 in L1 (currently in {input_0_memory})"
            )
        ...
        if inner_dim_block is None and out_h is None and out_w is None:
            ...
            if inner_dim_block is not None:
                if inner_dim_block < 2:
                    advice.append(
                        f"in0_block_w={inner_dim_block} is small, try in0_block_w=2 or above"
                    )
                    all_good = False
            else:
...
def generate_stacked_report(
    rows, visible_headers, stack_by_input0_layout: bool = False
):
    ...
    data = {
        header: [row[header].raw_value for row in rows] for header in visible_headers
    }
    df = pd.DataFrame(data)

    if stack_by_input0_layout:
        df["OP Code Joined"] = (
            df["OP Code"].str.split().str[0]
            + " (in0:"
            + df["Input 0 Memory"].str.split("_").str[-2].str.lower()
            + "_"
            + df["Input 0 Memory"].str.split("_").str[-1].str.lower()
            + ")"
        )
    else:
...
def plot_stacked_report(
    stacked_df: pd.DataFrame, output_file: str, threshold: float = 0.02
):
    ...
    for i, row in stacked_df.iterrows():
        ...
        bar = plt.bar(
            1,
            row["Device_Time_Sum_us"],
            width,
            label=row["OP Code Joined"],
            bottom=bottom,
            color=color,
        )
    ...
    while max(len(block_by_device[device_id]) for device_id in device_ids) > 0:
        ...
        for device_id in device_ids:
            if not len(block_by_device[device_id]):
                print(
                    colored(
                        f"Warning: Device {device_id} is missing operation {op_name} at index {global_index}",
                        "yellow",
                    )
                )
                continue
        ...
        if (
            ...
            base_block["DEVICE KERNEL DURATION [ns]"] = (
                sum(device_kernel_durations) / len(device_kernel_durations)
                if device_kernel_durations
                else float("nan")
            )
            ...
            max_duration_block = max(
                blocks, key=lambda x: x[1]["DEVICE KERNEL DURATION [ns]"]
            )
...
def parse_args():
    ...
    parser.add_argument(
        "csv_file", type=str, help="Path to the performance report CSV file"
    )
    parser.add_argument(
        "--signpost",
        type=str,
        help="Specify a signpost to use for analysis",
        default=None,
    )
    parser.add_argument(
        "--ignore-signposts",
        action="store_true",
        help="Ignore all signposts and use the entire file for analysis",
    )
    parser.add_argument(
        "--min-percentage",
        type=float,
        default=0.5,
        help="Minimum percentage for coloring (default: 0.5)",
    )
    parser.add_argument(
        "--id-range",
        type=str,
        help="Show only rows with IDs in the specified range (e.g., '5-10', '31-', or '-12')",
    )
    parser.add_argument(
        "--color",
        action="store_true",
        help="Force colored output even when output is redirected",
    )
    parser.add_argument(
        "--no-color", action="store_true", help="Force output without color"
    )
    parser.add_argument(
        "--csv", type=str, help="Output filename for CSV format", metavar="OUTPUT_FILE"
    )
    parser.add_argument(
        "--no-advice",
        action="store_true",
        help="Only show the table section of the report",
    )
    parser.add_argument(
        "--tracing-mode", action="store_true", help="Do not sort when in tracing mode"
    )
    parser.add_argument(
        "--raw-op-codes", action="store_true", help="Include raw op codes in output"
    )
    parser.add_argument(
        "--no-host-ops", action="store_true", help="Do not include host ops in output"
    )
    parser.add_argument(
        "--no-stacked-report",
        action="store_true",
        help="Do not generate a stacked report",
    )
    parser.add_argument(
        "--no-stack-by-in0",
        action="store_true",
    ...
    parser.add_argument(
        "--stacked-csv",
        type=str,
        help="Output filename for the stacked report CSV; Defaults to OUTPUT_FILE_stacked.csv",
        metavar="STACKED_FILE",
    )
    ...
    try:
        ...
        print(
            colored(
                "Invalid --id-range format. Please use 'START-END', 'START-', or '-END'.",
                "red",
            )
        )
        exit(1)
...
def generate_perf_report(
    ...
    df["ORIGINAL_ROW"] = (
        df.index + 2
    )  # +2 to match Excel row numbers (1-based + header)
    ...
    if "HOST START TS" in df.columns and not tracing_mode:
        ...
        print(
            colored(
                "Warning: 'HOST START TS' column not found. CSV will not be sorted.",
                "yellow",
            )
        )
    ...
    if "DEVICE ID" in df.columns and df["DEVICE ID"].nunique() > 1:
        print(
            colored(
                f"Detected data from {df['DEVICE ID'].nunique()} devices. Merging device data...",
                "cyan",
            )
        )
    ...
    rows = [
        color_row(op_data, op_data["Total %"].raw_value, min_percentage)
        for op_data in rows
    ]
    ...
    if csv_output_file:
        ...
        with open(csv_output_file, "w") as f:
            ...
            for op_data in rows:
                row = {
                    header: op_data[header].raw_value
                    for header in all_headers
                    if header in op_data
                }
                if not no_advice:
                    advice = (
                        generate_matmul_advice(op_data) if is_matmul_op(op_data) else ""
                    )
        ...
        col_widths = [
            max(
                max(visible_length(str(row[header])) for row in rows),
    ...
    if not (no_stacked_report) and rows:
        stacked_report = generate_stacked_report(
            rows, visible_headers, not (no_stack_by_in0)
        )
        ...
        if stacked_report_file or csv_output_file:
            if not stacked_report_file:
                base_stacked_report_file = (
                    f"{os.path.splitext(csv_output_file)[0]}_stacked"
                )
            else:
                base_stacked_report_file = os.path.splitext(stacked_report_file)[0]
            print(
                colored(
                    f"Writing CSV stacked report to {base_stacked_report_file}.csv",
                    "cyan",
                )
            )
            ...
            print(
                colored(
                    f"Plotting PNG stacked report to {base_stacked_report_file}.png",
                    "cyan",
                )
            )
